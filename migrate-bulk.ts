...
function superAdvancedClean(html: string) {
    ...
    const cleanedLines = lines.filter(line => {
        ...
        // If it contains { } or starts with CSS-like selector, filter it
...
async function run() {
    console.log('--- Super Advanced Bulk Migration Started (Rows 1-50) ---');
    ...
    const endIdx = Math.min(51, jobsData.length);
    ...
    for (let idx = 1; idx < endIdx; idx++) {
        ...
        const description = superAdvancedClean(row[16] || ""); // Q (SUPER CLEAN)
        ...
        try {
            ...
            console.log(`- OK [No ${legacyId}] ${targetId} Super Cleaned.`);
        } catch (error) {
            console.error(`- ERROR ${targetId}:`, error);
        }
    }
    console.log('\n--- Super Advanced Migration Finished ---');
}
